...
class fault_injection:
    def __init__(self, model, h, w, batch_size, **kwargs):
        ...
        for param in self.ORIG_MODEL.modules():
            if isinstance(param, nn.Linear):
                handles_fc.append(param.register_forward_hook(self._save_output_size_fc))
    ...
    def _fi_state_reset(self):
        ...
        for i in range(len(self.HANDLES_FC)):
            self.HANDLES_FC[i].remove()
    def declare_weight_fi(self, **kwargs):
        ...
        corrupt_idx_Fc = [corrupt_kH, corrupt_kW]
        curr_layer = 0
        for name, param in self.CORRUPTED_MODEL.named_parameters():
            if "weight" in name and "features" in name:
                ...
                if curr_layer == corrupt_layer:
                    corrupt_idx_Fc = (
                        tuple(corrupt_idx_Fc)
                        if isinstance(corrupt_idx_Fc, list)
                        else corrupt_idx_Fc
                    )
                    orig_value = param.data[corrupt_idx_Fc].item()
                    if CUSTOM_INJECTION:
                        corrupt_value = CUSTOM_FUNCTION(param.data, corrupt_idx_Fc)
                    param.data[corrupt_idx_Fc] = corrupt_value
    ...
    def declare_neuron_fi(self, **kwargs):
        ...
        for param in self.CORRUPTED_MODEL.modules():
            if isinstance(param, nn.Conv2d):
                ...
                hook = INJECTION_FUNCTION if CUSTOM_INJECTION else self._set_value_fc
                self.HANDLES_FC.append(param.register_forward_hook(hook))
    ...
    def _set_value_fc(self, module, input, output):
        if type(self.CORRUPT_CONV) == list:
            inj_list = list(
                filter(
            ...
            for i in inj_list:
                ...
                logging.info(
                    ...
                    % (
                        ...
                        self.CORRUPT_W[i],
                        output[self.CORRUPT_BATCH[i]][[self.CORRUPT_W[i]]]
                    )
                )
            ...
            if self.get_curr_conv() == self.CORRUPT_CONV:
                logging.info(
                    ...
                    % (
                        self.CORRUPT_BATCH,
                        self.CORRUPT_W,
                        output[self.CORRUPT_BATCH][self.CORRUPT_W
                        ],
                    )
                )
                logging.info("Changing value to %d" % self.CORRUPT_VALUE)
                output[self.CORRUPT_BATCH][
                    self.CORRUPT_W
                ] = self.CORRUPT_VALUE
    ...
    def _save_output_size_fc(self,module,input,output):
        self.OUTPUT_SIZE_FC.append(list(output.size()))
