...
def fault_injection(model, Conv_n, max_value, total_param=-1, probability=10 ** -5, seed=0):
    ...
    for i in range(int(total_param)):
        if bernouli[i] == 1:
            number_fault += 1
            fault_model_object = pfi_core(model, 32, 32, 64, use_cuda=False, c=3)
            if Conv_n > 4:
            ...
            if orig_value > 100000 or torch.abs(orig_value).isinf() or orig_value < -100000:
                continue
...
def return_activation_output(name_module, test_loader, model, DEVICE):
    b = 0
    result = []
    ...
    for data, label in test_loader:
        ...
        result.append(X.to("cpu").detach().numpy())
        if b == 20:
            break
        b += 1
    return result
...
def accuracy_vs_faultrate(fault_rate, model, test_losder, name_layer, conv_number, seed):
    ...
    acc = compute_accuracy(fault_model, test_loader, 'cpu')
    return acc
...
def output_act_model(model, name_activation, test_loader, device):
    result = return_activation_output(name_activation, test_loader, model, device)
    out = np.array(result).flatten()
    out = out[out < 1E308]
    return np.max(out)


def output_act_model_without_flatten(model, name_activation, test_loader, device):
    result = return_activation_output(name_activation, test_loader, model, device)
    out = np.array(result)
...
class bounded_relu(nn.Module):
    ...
    def forward(self, input):
        return torch.max(torch.tensor(0), torch.min(torch.tensor(self.bound), input))


def model_with_treshold(model, number_activation, T, features_class, device):
    model = model.to(device)
...
def main_search_alg(model, N, M, name_activation, act_number, test_loader, fault_rates, name_layer, conv_number,
                    delta_b, seed):
    i = 1
    counter = 1
...
def model_with_bound_layer_act_max(model, name_activation, number_activation, test_loader, device):
    for act_name, act_number in zip(name_activation, number_activation):
        act_max = output_act_model(model, act_name, test_loader, device)
        model = model_with_treshold(model, act_number, act_max, act_name.split('.')[0], 'cpu')
...
def model_with_bound_neuron_act_max(model, name_activation, number_activation, test_loader, device):
    for act_name, act_number in zip(name_activation, number_activation):
        act_max = output_act_model(model, act_name, test_loader, device)
        model = model_with_treshold(model, act_number, act_max, act_name.split('.')[0], 'cpu')
    return model.to(device)
...
if __name__ == '__main__':
    ...
    model, acc = train_eval_alexnet(10, train_loader, test_loader, training=False)
    model = model.to(Device)
    ...
    conv_number = 3
    orig_model = copy.deepcopy(model)
    new_model_max = model_with_bound_layer_act_max(model, activation_name, activation_number, test_loader, Device)
    ...
    for f in fault_rates:
        for i in range(1000):
            ac += accuracy_vs_faultrate(f, model, test_loader, name_layer, conv_number, i, Device)
            acm += accuracy_vs_faultrate(f, new_model_max, test_loader, name_layer, conv_number, i, Device)
            acf += accuracy_vs_faultrate(f, new_model_neuron_act, test_loader, name_layer, conv_number, i, Device)
            model = copy.deepcopy(orig_model)
            new_model_max = copy.deepcopy(orig_model_bound_max)
        ...
        acf = 0
    plt.plot(fault_rates_i, acc_unbound, 'bo', fault_rates_i, acc_unbound, 'r', fault_rates_i, acc_bound_max,
             'ro', fault_rates_i, acc_bound_max, 'b', fault_rates_i, acc_bound_flip, 'r-', fault_rates_i,
             acc_bound_flip, 'g')
    plt.xticks(fault_rates_i, fault_rates)
