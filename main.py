...
class bounded_relu(nn.Module):
    ...
    def forward(self, input):
        return torch.maximum(torch.tensor(0),torch.minimum(torch.tensor(self.bound),input))
...
if __name__ == '__main__':
    ...
    # new_model_neuron_act = model_with_bound_neuron_act_max(model, activation_name,activation_number,test_loader,Device)
    # orig_model_bound_max = copy.deepcopy(new_model_max)
    ...
    for tr in T:
        print(tr)
        model = model_with_custom_bound(model,activation_name,activation_number,test_loader,'cpu',tr)
        ac = compute_accuracy(model,test_loader,'cpu')
        acc.append(acc)
        model = copy.deepcopy(orig_model)
    plt.plot(T,acc,'bo',T,acc,'r')    
    plt.show()
