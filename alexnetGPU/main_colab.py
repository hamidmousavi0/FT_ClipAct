...
if __name__ == '__main__':
    ...
    acc_f = compute_accuracy(fault_model,test_loader,DEVICE)
    orig_model = copy.deepcopy(fault_model)
    acc = []
    acc_original=[]
    acc_fault=[]
    for tr in T:
        print(tr)
        fault_model = model_with_custom_bound(fault_model,activation_name,activation_number,test_loader,DEVICE,tr)
        ac = compute_accuracy(fault_model,test_loader,DEVICE)
        acc.append(ac)
        acc_original.append(accuracy)
        acc_fault.append(acc_f)
        fault_model = copy.deepcopy(orig_model)
    plt.plot(T,acc_original,'b',T,acc_fault,'g',T,acc,'r')    
    plt.xticks(np.range(0,60),2)
    plt.yticks(np.range(0,1),0.1)
