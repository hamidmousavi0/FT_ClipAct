...
if __name__ == '__main__':
    ...
    model ,accuracy= train_eval_alexnet(1,train_loader,test_loader,training=False)
    ...
    activation_name='features.4'#['features.1','features.4','features.7','features.9',
                #'features.11','classifier.2','classifier.5']
    activation_number ='4'#['1','4','7','9','11','2','5']
    # layer_name ='features.3' #['features.0', 'features.3', 'features.6', 'features.8',
    #               'features.10', 'classifier.1', 'classifier.4']
    fault_rates = 10**-4
    seed=1
    conv_number = np.random.randint(2,size=1)[0]
    if conv_number==0:
    ...
    if max_value < 1:
        max_value = 1
    print(total_params) 
    model = model_with_custom_bound_single(model,'features.1','1',test_loader,DEVICE,2)
    orig_model = copy.deepcopy(model) 
    ...
    for tr in T:
        ...
        acc_bound_list.append(acc_bound/500)
        if acc_bound/500 >acc_max:
            print(acc_bound/500,tr)
            acc_max=acc_bound/500
        model = copy.deepcopy(orig_model)
    plt.plot(T,acc_original,'b',T,acc_fault,'g',T,acc_bound_list,'r')    
    plt.xticks(np.arange(0,20,2))
    plt.yticks(np.arange(0,1,0.1))
    plt.savefig("10_4_1.png")
