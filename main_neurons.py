...
def output_act_model_without_flatten(model,name_activation,test_loader):
    result = return_activation_output(name_activation, test_loader, model)
    out1 = np.array(result)
    # print(out1.shape)
    out = out1.reshape(out1.shape[0] * out1.shape[1], -1)
    return out.max(axis=0).reshape(out1.shape[2:])
...
class bounded_relu(nn.Module):
    ...
    def forward(self, input):
        A= torch.min(torch.tensor(self.bound).float().to('cuda'),input.to('cuda'))
        A[A==torch.tensor(self.bound).to('cuda')]=0
        print(A.shape)
        return torch.max((torch.tensor(0).float()).to('cuda'),torch.tensor(A))
def model_with_treshold(model,number_activation,T,features_class):
    ...
    return model
class bounded_relu_mean(nn.Module):
    def __init__(self, bound = 1,mean=0):
        super().__init__()
        self.bound = bound
        self.mean = mean
    def forward(self, input):
        A= torch.min(torch.tensor(self.bound).float().to('cuda'),input.to('cuda'))
        A[A==torch.tensor(self.bound).to('cuda')]=torch.tensor(self.mean).to('cuda')
        return torch.max((torch.tensor(0).float()).to('cuda'),torch.tensor(A))    
def model_with_treshold_mean(model,number_activation,T,features_class,mean):
    for name, module in model.named_children():
        if isinstance(module,nn.Sequential):
            for name, module in module.named_children():
                if name==number_activation: # should be number
                    # print(model._modules[features_class][int(number_activation)])
                    model._modules[features_class][int(number_activation)] = bounded_relu_mean(T,mean)
...
def  model_with_bound_neuron_act_max(model,name_activation,number_activation,test_loader):
    for act_name, act_number in zip(name_activation, number_activation):
        act_max = output_act_model_without_flatten(model, act_name, test_loader)
        model = model_with_treshold(model, act_number, act_max, act_name.split('.')[0])
    return model
...
if __name__ == '__main__':
    ...
    model ,accuracy= train_eval_alexnet(5,train_loader,test_loader,training=True)
    ...
    # model = model_with_treshold(model, '1', 100, name_activation.split('.')[0])
    # act_max=output_act_fault_model(fault_rate, model, name_layer, conv_number, name_activation,seed)
    # print(act_max)
    ############################################################################
    #################### AUC VS TRESHOLD##################################
    # conv_number = 0
    ...
    fault_rates = 10**-6
    seed=1
    conv_number = np.random.randint(7,size=1)[0]
    orig_model = copy.deepcopy(model)
    new_model_max = model_with_bound_layer_act_max(model, activation_name,activation_number,test_loader)
    model = copy.deepcopy(orig_model)
    ...
    for i in range(iteration):
        ...
        conv_number = np.random.randint(7,size=1)[0]
        for name, param in model.named_parameters():
            if "weight" in name and layer_name[conv_number] in name:
                total_params = param.numel()
                max_value = param.max()
        if max_value < 1:
            max_value = 1
        print(total_params) 
        fault_model_max= fault_injection(new_model_max, conv_number, max_value, total_params, fault_rates,seed) 
        fault_model_neuron= fault_injection(new_model_neuron_act, conv_number, max_value, total_params, fault_rates,seed) 
        new_model_max = copy.deepcopy(orig_model_bound_max)
        new_model_neuron_act = copy.deepcopy(orig_model_bound_neuron_act)
        acm += compute_accuracy(fault_model_max,test_loader,DEVICE)
        acf += compute_accuracy(fault_model_neuron,test_loader,DEVICE)
    print("acc_bound_max={}".format(acm/iteration))
    print("acc_bound_neuron={}".format(acf/iteration))
...
#################max output model###################################3
    # activation_name=['features.1','features.4','features.7','features.9',
    #             'features.11','classifier.2','classifier.5']
    # for act_name in activation_name:            
    #     out_mean = output_act_model(model,act_name,test_loader)
    #     print(out_mean)
##############################compare accuracy for bound########################
    # activation_name=['features.1','features.4','features.7','features.9',
    #             'features.11','classifier.2','classifier.5']
    # activation_number =['1','4','7','9','11','2','5']
    # layer_name =['features.0', 'features.3', 'features.6', 'features.8',
    ...
    # #     max_value = 1
    # # print(total_params) 
    # bound=[6.21,4.26,2.013,2.02,1.72,1.9,2.57]
    # mean =[0.17466973,0.069095545,0.056486677,0.07438989,0.036758155,0.0835989,0.12999293]
    # i=0
    # j=0
    # k=0
    # orig_model = copy.deepcopy(model) 
    # for act_name,act_number in zip(activation_name,activation_number):
    #     model_bound_2_zero = model_with_treshold(model, act_number, 2, act_name.split('.')[0])
    # model = copy.deepcopy(orig_model) 
    # for act_name,act_number in zip(activation_name,activation_number):
    #     model_bound_2_mean = model_with_treshold_mean(model, act_number, 2, act_name.split('.')[0],mean[j])
    #     j+=1
    # model = copy.deepcopy(orig_model)    
    # for act_name,act_number in zip(activation_name,activation_number):
    #     model_bound_max_zero = model_with_treshold(model, act_number, bound[i], act_name.split('.')[0])
    #     i+=1
    # i=0    
    # model = copy.deepcopy(orig_model) 
    # for act_name,act_number in zip(activation_name,activation_number):
    #     model_bound_max_mean = model_with_treshold_mean(model, act_number, bound[i], act_name.split('.')[0],mean[k])
    #     i+=1  
    #     k+=1
    # model = copy.deepcopy(orig_model)  
    # model_orig = copy.deepcopy(orig_model)  
    # model_bound_2_zero_orig = copy.deepcopy(model_bound_2_zero) 
    # model_bound_2_mean_orig = copy.deepcopy(model_bound_2_mean)
    # model_bound_max_zero_orig = copy.deepcopy(model_bound_max_zero)  
    # model_bound_max_mean_orig = copy.deepcopy(model_bound_max_mean)  
    # acc_bound_list = []
    # acc_original=[]
    # acc_fault=[]   
    # acc_bound2_zero=0
    ...
    #         max_value = 1
    #     print(total_params) 
    #     fault_model_bound2_zero = fault_injection(model_bound_2_zero, conv_number, max_value, total_params, fault_rates,seed) 
    #     fault_model_bound2_mean = fault_injection(model_bound_2_mean, conv_number, max_value, total_params, fault_rates,seed)
    #     fault_model_bound_max_zero = fault_injection(model_bound_max_zero, conv_number, max_value, total_params, fault_rates,seed) 
    #     fault_model_bound_max_mean = fault_injection(model_bound_max_mean, conv_number, max_value, total_params, fault_rates,seed)
    #     fault_model = fault_injection(model, conv_number, max_value, total_params, fault_rates,seed) 
    #     acc_bound2_zero += compute_accuracy(fault_model_bound2_zero,test_loader,DEVICE)
    #     acc_bound2_mean += compute_accuracy(fault_model_bound2_mean,test_loader,DEVICE)
    #     acc_max_zero += compute_accuracy(fault_model_bound_max_zero,test_loader,DEVICE)
    #     acc_max_mean += compute_accuracy(fault_model_bound_max_mean,test_loader,DEVICE)
    #     acc_orig += compute_accuracy(fault_model,test_loader,DEVICE)
    #     model_bound_2_zero = copy.deepcopy(model_bound_2_zero_orig)
    #     model_bound_2_mean = copy.deepcopy(model_bound_2_mean_orig)
    #     model_bound_max_zero = copy.deepcopy(model_bound_max_zero_orig)
    #     model_bound_max_mean = copy.deepcopy(model_bound_max_mean_orig)
    #     model = copy.deepcopy(model_orig)
    
    # acc_bound2_zero = acc_bound2_zero/iteration
    # acc_bound2_mean = acc_bound2_mean/iteration
    # acc_max_zero = acc_max_zero/iteration
    # acc_max_mean= acc_max_mean/iteration
    # acc_orig = acc_orig/iteration
    # print("acc bound 2 zero={}".format(acc_bound2_zero))
    # print("acc bound 2 mean={}".format(acc_bound2_mean))
    # print("acc max zero={}".format(acc_max_zero))
    # print("acc max mean={}".format(acc_max_mean))
    # print("acc unbound={}".format(acc_orig))
